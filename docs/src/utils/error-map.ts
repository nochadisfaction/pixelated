...
const flattenErrorPath = (errorPath: (string | number)[]): string => errorPath.join('.')

const prefix = (key: string, msg: string): string => (key.length ? `**${key}**: ${msg}` : msg)
...
function processParsedData(parsedData: z.SafeParseReturnType<any, any>, message: string): any {
...
function unionExpectedVals(expectedVals: Set<unknown>): string {
  return [...expectedVals].map(expectedVal => stringify(expectedVal)).join(' | ')
}
...
function stringify(val: unknown): string {
  return JSON.stringify(val, null, 1).split(newlinePlusWhitespace).join(' ')
}
