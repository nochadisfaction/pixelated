...
async function runPreDeploymentChecks(environment) {
  ...
  if (status.files.length > 0) {
    ...
    if (environment === 'production') {
      const proceed = await confirmPrompt('Continue with production deployment despite uncommitted changes?')
      if (!proceed) {
  ...
  if (fs.existsSync(path.join(projectRoot, 'scripts/clean-credentials.js'))) {
    try {
      runCommand('node scripts/clean-credentials.js --check-only', 'Security credential check', true)
      console.log(chalk.green('✅ Security checks passed'))
    } catch (error) {
      console.warn(chalk.yellow('⚠️ Security checks failed - fixing automatically'))
  ...
  if (environment === 'production') {
    try {
      ...
      const proceed = await confirmPrompt('Type checks failed. Continue with deployment?')
      if (!proceed) {
  ...
  try {
    if (environment === 'production') {
      ...
      if (process.env.VERCEL_TOKEN || process.env.CI) {
        runCommand('vercel deploy --prod --yes', 'Deploying to Vercel Production')
      } else {
    ...
    console.error(chalk.red(`❌ Deployment to ${environment} failed: ${error.message}`))
    throw error
  }
}
...
async function deploy(environment, options = {}) {
  console.log(chalk.cyan.bold(`🚀 Starting deployment to ${environment.toUpperCase()}`))
  ...
  try {
    ...
    if (environment === 'production') {
      const result = await createProductionTag({
        message: options.message || `Production deployment ${new Date().toISOString()}`
      })
      deploymentTag = result.mainTag
    } else {
      deploymentTag = await createStagingTag({
        message: options.message || `Staging deployment ${new Date().toISOString()}`
      })
    }
    ...
    if (environment === 'production') {
      ...
      try {
        const healthCheck = runCommand('curl -f https://pixelated.vercel.app/api/health || echo "Health check endpoint not available"', 'Health check', true)
...
async function rollback(environment) {
  console.log(chalk.cyan.bold(`🔄 Starting rollback for ${environment.toUpperCase()}`))
  ...
  try {
    ...
    const proceed = await confirmPrompt(`Confirm rollback from ${validation.current} to ${validation.previous}?`)
    if (!proceed) {
    ...
    // Create rollback tag
    const rollbackTag = await createRollbackTag(environment, validation.previous)
    
    // Return to original branch
    const originalBranch = await git.branch()
    if (originalBranch.current !== originalBranch.all.find(b => b.includes('HEAD'))) {
