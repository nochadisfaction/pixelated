    ...
    try:
        ...
        if parsed.netloc in ['www.youtube.com', 'youtube.com', 'm.youtube.com']:
            ...
            if not (parsed.path.startswith('/watch') or 
                   parsed.path.startswith('/playlist')):
                return False
...
def download_playlist(url: str, output_dir: str, audio_format: str = "mp3"):
    ...
    try:
        ...
        result = subprocess.run(
            ...
            timeout=3600, 
            shell=False,
            cwd=os.path.dirname(output_dir)  # Set safe working directory
        )

        if result.returncode != 0:
            logging.error(f"yt-dlp failed with return code {result.returncode}")
            # Don't log full stderr as it might contain sensitive info
            if result.stderr:
...
def main():
    ...
    parser.add_argument("--url-file", type=str, help="File containing list of playlist/video URLs (one per line)")
    parser.add_argument("--output-dir", type=str, default="voice_data", help="Directory to save audio files")
