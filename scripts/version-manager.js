  ...
  switch (type) {
    ...
    case 'prerelease':
      if (version.prerelease) {
        ...
        version.prerelease = match ? `${match[1]}.${parseInt(match[2]) + 1}` : `${version.prerelease}.1`
      } else {
...
function generateChangelog(fromVersion, toVersion) {
  try {
    ...
    const commits = runGitCommand(`git log ${range} --pretty=format:"%h %s" --no-merges`)
    ...
    lines.forEach(line => {
      ...
      commit.includes('feat:') || commit.includes('feature:') ? categorized.features.push(commit) :
      commit.includes('fix:') || commit.includes('bug:') ? categorized.fixes.push(commit) :
      categorized.other.push(commit)
    })
    
    let changelog = `## Version ${toVersion}\n\n`
    
    changelog += categorized.features.length > 0 ? 
      '### 🚀 Features\n' + categorized.features.map(commit => `- ${commit}`).join('\n') + '\n\n' : ''
    
    changelog += categorized.fixes.length > 0 ? 
      '### 🐛 Bug Fixes\n' + categorized.fixes.map(commit => `- ${commit}`).join('\n') + '\n\n' : ''
    
    changelog += categorized.other.length > 0 ? 
...
async function main() {
  ...
  for (let i = 2; i < args.length; i++) {
    ...
    if (arg.startsWith('--message=')) {
      ...
      options.skipChangelog = arg === '--skip-changelog' ? true : options.skipChangelog
      options.skipCommit = arg === '--skip-commit' ? true : options.skipCommit
      options.skipPush = arg === '--skip-push' ? true : options.skipPush
      options.skipDeploymentTags = arg === '--skip-deployment-tags' ? true : options.skipDeploymentTags
    }
  }
    ...
    switch (command) {
      case 'bump':
        if (!['major', 'minor', 'patch', 'prerelease'].includes(type)) {
          throw new Error('Invalid version type. Use: major, minor, patch, prerelease')
        }
      ...
      case 'release':
        if (!['major', 'minor', 'patch', 'prerelease'].includes(type)) {
          throw new Error('Invalid release type. Use: major, minor, patch, prerelease')
        }
