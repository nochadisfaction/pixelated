...
describe('BiasDashboard', () => {
  ...
  it('renders loading state initially', () => {
    ...
    expect(screen.getByText('Loading bias detection dashboard...')).toBeInTheDocument();
  });
  ...
  it('handles WebSocket errors gracefully', async () => {
    ...
    const onError = mockWs.addEventListener.mock.calls.find(
      call => call[0] === 'error'
    )[1];
    onError(new Event('error'));

    expect(screen.getByText('Error connecting to live updates')).toBeInTheDocument();
  });
  ...
  it('handles chart interactions correctly', async () => {
    ...
    // Test chart tooltips
    const chartElement = screen.getByText(/bias score trends/i).closest('.recharts-wrapper');
    if (chartElement) {
  ...
  it('clears all filters when clear button is clicked', async () => {
    ...
    expect(biasScoreSelect.value).toBe('all');
    expect(alertLevelSelect.value).toBe('all');
  });
  ...
  it('updates chart data when filters are applied', async () => {
    ...
    expect(screen.getByText(/bias score trends \(\d+ data points\)/i)).toBeInTheDocument();
  });
  ...
  it('handles notification settings changes', async () => {
    ...
    expect(criticalAlertsCheckbox.checked).toBe(false);
  });
  ...
  it('handles test notification sending', async () => {
    ...
    await waitFor(() => {
      expect(alertSpy).toHaveBeenCalledWith('Test notification sent successfully!');
    });
  ...
  it('handles alert selection and deselection', async () => {
    ...
    const individualAlertCheckbox = alertCheckboxes.find(cb => cb !== screen.getByLabelText(/select all/i));
  ...
  it('handles alert notes addition', async () => {
    ...
    // Mock window.prompt
    const promptSpy = vi.spyOn(window, 'prompt').mockReturnValue('Test note for alert');
  ...
  it('handles alert escalation with notes', async () => {
    ...
    // Mock window.prompt
    const promptSpy = vi.spyOn(window, 'prompt').mockReturnValue('Escalation reason');
  ...
  describe('Data Export Functionality', () => {
    ...
    it('handles export data functionality', async () => {
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith('/api/bias-detection/export?format=json', {
          method: 'GET'
        });
      });
    ...
    it('closes export dialog when cancel is clicked', async () => {
      ...
      expect(screen.queryByText(/export dashboard data/i)).not.toBeInTheDocument();
    });
  ...
  describe('Accessibility Features', () => {
    ...
    it('provides proper ARIA labels and descriptions', async () => {
      ...
      const notificationButton = screen.getByLabelText(/open notification settings/i);
    ...
    it('supports high contrast mode', async () => {
      ...
      const container = screen.getByText(/bias detection dashboard/i).closest('div');
  ...
  describe('Enhanced WebSocket Functionality', () => {
    ...
    it('handles real-time bias alert updates', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/real-time high bias detected/i)).toBeInTheDocument();
      });
    });
