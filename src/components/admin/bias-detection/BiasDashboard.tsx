...
export const BiasDashboard: React.FC<BiasDashboardProps> = ({
  ...
  const filterDataByTimeRange = useCallback((data: any[], timeRange: string) => {
    ...
    const endTime = timeRange === 'custom' && customDateRange.end 
  ...
  const handleAlertAction = useCallback(async (alertId: string, action: AlertAction['type'], notes?: string) => {
    try {
      ...
      logger.error('Failed to perform alert action', { error: err, alertId, action });
  ...
  const handleBulkAlertAction = useCallback(async (alertIds: string[], action: AlertAction['type']) => {
    try {
      const promises = alertIds.map(alertId => handleAlertAction(alertId, action));
      await Promise.all(promises);
      setSelectedAlerts(new Set()); // Clear selection
      logger.info(`Bulk ${action} completed`, { count: alertIds.length });
    } catch (err) {
      logger.error('Failed to perform bulk alert action', { error: err, action, count: alertIds.length });
    }
  }, [handleAlertAction]);
      ...
      try {
        ...
        ws.onopen = () => {
          ...
          ws.send(JSON.stringify({
            type: 'subscribe',
            channels: ['bias_alerts', 'session_updates', 'metrics_updates', 'trends_updates'],
            filters: {
        ...
        ws.onclose = (event) => {
          ...
          if (reconnectAttempts < maxReconnectAttempts) {
            ...
            announceToScreenReader('Live updates failed to reconnect. Please refresh the page.');
          }
        };
  ...
  }, [enableRealTimeUpdates, selectedTimeRange, biasScoreFilter, alertLevelFilter, announceToScreenReader]);
  ...
  }, [selectedTimeRange, biasScoreFilter, alertLevelFilter, selectedDemographicFilter]);
  ...
  const exportDataWithOptions = async () => {
    try {
      ...
      logger.error('Export failed', { error: errorMessage, exportParams: { format: exportFormat, dataTypes: exportDataTypes } });
  ...
  const getResponsiveGridCols = useCallback((maxCols: number) => {
    switch (screenSize) {
      case 'mobile':
        return 1;
      case 'tablet':
  ...
  return (
    <div className={`p-6 space-y-6 ${className} ${highContrast ? 'high-contrast' : ''}`}>
      ...
      {/* Header */}
      <header className={`flex ${isMobile ? 'flex-col space-y-4' : 'flex-row items-center justify-between'}`}>
        <div>
          ...
          <p className={`text-muted-foreground ${isMobile ? 'text-sm' : ''}`}>
            ...
            {enableRealTimeUpdates && (
              <span className={`${isMobile ? 'block' : 'ml-2'} ${connectionStatus.color} ${connectionStatus.pulse ? 'animate-pulse' : ''}`}>
        ...
        <div className={`flex ${isMobile ? 'flex-col space-y-2' : 'items-center space-x-2'} ${isMobile ? 'w-full' : ''}`}>
          <Button
            variant="outline"
            size={isMobile ? "default" : "sm"}
            onClick={() => {
              setAutoRefresh(!autoRefresh);
              announceToScreenReader(`Auto-refresh ${!autoRefresh ? 'enabled' : 'disabled'}`);
            }}
            className={isMobile ? 'w-full justify-start' : ''}
            aria-label={`Auto-refresh is currently ${autoRefresh ? 'on' : 'off'}. Click to ${autoRefresh ? 'disable' : 'enable'}.`}
          >
            <Activity className={`h-4 w-4 ${isMobile ? 'mr-2' : 'mr-2'} ${autoRefresh ? 'text-green-500' : 'text-gray-500'}`} />
          ...
          >
            <RefreshCw className={`h-4 w-4 ${isMobile ? 'mr-2' : 'mr-2'} ${loading ? 'animate-spin' : ''}`} />
            Refresh
          </Button>

          {/* WebSocket Reconnect Button - Show when connection failed */}
          {enableRealTimeUpdates && (wsConnectionStatus === 'error' || wsConnectionStatus === 'disconnected') && (
            <Button
              variant="outline"
              size={isMobile ? "default" : "sm"}
          ...
          <Button
            ...
            onClick={() => {
              ...
              announceToScreenReader(`Notification settings ${!showNotificationSettings ? 'opened' : 'closed'}`);
            }}
            className={isMobile ? 'w-full justify-start' : ''}
                ...
                <div className="space-y-3">
                  <label className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => updateNotificationSettings({ inAppEnabled: e.target.checked })}
                      className="rounded"
                    />
                ...
                <div className="space-y-3">
                  <label className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => updateNotificationSettings({ criticalAlerts: e.target.checked })}
                      className="rounded"
                    />
                  ...
                  <label className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => updateNotificationSettings({ highAlerts: e.target.checked })}
                      className="rounded"
                    />
                  ...
                  <label className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => updateNotificationSettings({ mediumAlerts: e.target.checked })}
                      className="rounded"
                    />
                  ...
                  <label className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => updateNotificationSettings({ lowAlerts: e.target.checked })}
                      className="rounded"
                    />
            ...
            <div className="mt-6 pt-4 border-t">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  disabled={!notificationSettings.emailEnabled && !notificationSettings.smsEnabled && !notificationSettings.inAppEnabled}
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportDataTypes(prev => ({ ...prev, summary: e.target.checked }))}
                      className="rounded"
                    />
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportDataTypes(prev => ({ ...prev, alerts: e.target.checked }))}
                      className="rounded"
                    />
                    <span>Alerts</span>
                  </label>
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportDataTypes(prev => ({ ...prev, trends: e.target.checked }))}
                      className="rounded"
                    />
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportDataTypes(prev => ({ ...prev, demographics: e.target.checked }))}
                      className="rounded"
                    />
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportDataTypes(prev => ({ ...prev, sessions: e.target.checked }))}
                      className="rounded"
                    />
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportDataTypes(prev => ({ ...prev, recommendations: e.target.checked }))}
                      className="rounded"
                    />
                ...
                <div className="space-y-3">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportFilters(prev => ({ ...prev, applyCurrentFilters: e.target.checked }))}
                      className="rounded"
                    />
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportFilters(prev => ({ ...prev, includeArchived: e.target.checked }))}
                      className="rounded"
                    />
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Button
                    ...
                    disabled={exportProgress.isExporting || !Object.values(exportDataTypes).some(Boolean)}
                  >
                    {exportProgress.isExporting ? (
                      <>
                        <RefreshCw className="h-4 w-4 mr-2 animate-spin" />
        ...
        <CardContent>
          ...
          <div className="mt-4 p-3 bg-muted rounded-md">
            <p className="text-sm text-muted-foreground">
              ...
              {selectedTimeRange !== '24h' && ` Time: ${timeRangeOptions.find(o => o.value === selectedTimeRange)?.label}`}
              {biasScoreFilter !== 'all' && ` • Bias: ${biasScoreFilter}`}
              {alertLevelFilter !== 'all' && ` • Alerts: ${alertLevelFilter}`}
              {selectedDemographicFilter !== 'all' && ` • Demographics: ${selectedDemographicFilter}`}
              {selectedTimeRange === '24h' && biasScoreFilter === 'all' && alertLevelFilter === 'all' && selectedDemographicFilter === 'all' && ' None'}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      {/* Critical Alerts */}
      {filteredAlerts.filter(alert => alert.level === 'critical' || alert.level === 'high').length > 0 && (
        <Alert 
          variant="error"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${getBiasScoreColor(
              filteredSessions.length > 0 
                ? filteredSessions.reduce((sum, session) => sum + (session.overallBiasScore || 0), 0) / filteredSessions.length
                : summary.averageBiasScore
            )}`}>
              {filteredSessions.length > 0 
                ? ((filteredSessions.reduce((sum, session) => sum + (session.overallBiasScore || 0), 0) / filteredSessions.length) * 100).toFixed(1)
                : (summary.averageBiasScore * 100).toFixed(1)
              }%
            </div>
            <Progress 
              value={filteredSessions.length > 0 
                ? (filteredSessions.reduce((sum, session) => sum + (session.overallBiasScore || 0), 0) / filteredSessions.length) * 100
      ...
      >
        <Tabs defaultValue="trends" className="w-full">
          <TabsList className={`grid w-full ${isMobile ? 'grid-cols-2' : isTablet ? 'grid-cols-3' : 'grid-cols-5'} ${isMobile ? 'h-auto' : ''}`}>
            <TabsTrigger 
              value="trends"
          ...
          <TabsContent value="trends" className="space-y-6">
            ...
            <div className={`grid grid-cols-1 ${getResponsiveGridCols(2) === 2 ? 'lg:grid-cols-2' : ''} gap-6`}>
              <Card>
                <CardHeader>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <label className="flex items-center space-x-2">
                        <input
                          ...
                          onChange={(e) => e.target.checked ? selectAllAlerts() : clearAlertSelection()}
                          className="rounded"
                        />
                      ...
                      {selectedAlerts.size > 0 && (
                        <div className="flex items-center space-x-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleBulkAlertAction(Array.from(selectedAlerts), 'acknowledge')}
                          >
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleBulkAlertAction(Array.from(selectedAlerts), 'dismiss')}
                          >
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleBulkAlertAction(Array.from(selectedAlerts), 'archive')}
                          >
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Badge variant="destructive">
                        {filteredAlerts.filter(a => a.level === 'critical' || a.level === 'high').length} high priority
                      </Badge>
                    </div>
                  </div>
                </CardContent>
                                ...
                                {lastAction && (
                                  <div className="mt-2 flex items-center space-x-2">
                                    <Badge variant="outline" className="text-xs">
                                      {lastAction.type === 'acknowledge' && <Check className="h-3 w-3 mr-1" />}
                                      {lastAction.type === 'dismiss' && <X className="h-3 w-3 mr-1" />}
                                      {lastAction.type === 'archive' && <Archive className="h-3 w-3 mr-1" />}
                                      {lastAction.type === 'escalate' && <AlertTriangle className="h-3 w-3 mr-1" />}
                                      {lastAction.type.charAt(0).toUpperCase() + lastAction.type.slice(1)}d
                                    </Badge>
