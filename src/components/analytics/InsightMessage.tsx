...
const getColorWithOpacity = (color: string, opacity: number): string => {
  const template = COLOR_MAP[color as keyof typeof COLOR_MAP] || 'rgba(0, 0, 0, $opacity)';
...
export const InsightMessage: React.FC<InsightMessageProps> = ({ summary, trend }) => {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
        <span aria-hidden="true" style={{ marginRight: '8px' }}>{icon}</span>
