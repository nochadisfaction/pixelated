...
export const ProgressDataDisplay: React.FC<ProgressDataDisplayProps> = ({
  ...
  return (
    ...
    >
      <h4 id="data-title" className={styles.chartTitle}>{title}</h4>
      {hasValidData ? (
        <>
          <p id="data-description" className={styles.chartDescription}>Data for: {formattedLabels}</p>
          ...
          <div className={styles.dataRow}>
            ...
            ></span>
            <p className={styles.dataText}>
              <span className={styles.dataLabel}>Benchmark ({benchmarkLabel}):</span> {formattedBenchmarkData}
            </p>
          </div>
          
          {/* Additional screen reader context that explains the data comparison */}
          <div className="sr-only" aria-live="polite">
            This data display compares user data {formattedUserData} with benchmark data for {benchmarkLabel} {formattedBenchmarkData} across the following categories: {formattedLabels}.
          </div>
        </>
      ) : (
        <p className={styles.noDataMessage}>No valid data available to display</p>
      )}
    </div>
  );
}; 