...
describe('BiasDetectionAuditLogger', () => {
  ...
  describe('Data Access Logging', () => {
    it('should log data access with proper sensitivity levels', async () => {
      ...
      expect(entry.details.dataAccessLog.dataType).toBe('demographics');
      expect(entry.details.dataAccessLog.dataIds).toEqual(['demo-1', 'demo-2']);
      expect(entry.details.dataAccessLog.approvedBy).toBe('supervisor-456');
    });
  ...
  describe('Data Sanitization', () => {
    it('should sanitize high-sensitivity data', async () => {
      ...
      expect(entry.details.demographics.region).toBe('REDACTED');
