...
class InMemoryAuditStorage implements AuditStorage {
  ...
  async retrieve(filters: AuditLogFilters): Promise<AuditLogEntry[]> {
    ...
    if (filters.actionCategory) {
      filtered = filtered.filter(e => e.action.category === filters.actionCategory);
    }
    if (filters.resource) {
      filtered = filtered.filter(e => e.resource === filters.resource);
    }
    if (filters.sensitivityLevel) {
      filtered = filtered.filter(e => e.action.sensitivityLevel === filters.sensitivityLevel);
    }
  ...
  async count(filters: AuditLogFilters): Promise<number> {
    const entries = await this.retrieve({ ...filters, limit: undefined, offset: undefined });
  ...
  async archive(beforeDate: Date): Promise<number> {
    ...
    logger.info('Audit log entries archived', { count: toArchive.length, beforeDate });
...
export class BiasDetectionAuditLogger {
  ...
  async logDataAccess(
    ...
    const action: AuditAction = {
      ...
      sensitivityLevel: this.determineSensitivityLevel(dataType, dataIds.length)
    };
  ...
  async logConfigurationChange(
    ...
    const action: AuditAction = {
      ...
      sensitivityLevel: update.changes.some(c => c.impact === 'critical' || c.impact === 'high') ? 'high' : 'medium'
    };
  ...
  async logAuthentication(
    ...
    const user: UserContext = {
      userId,
      email: userEmail,
      role: { id: 'unknown', name: 'viewer', description: 'Unknown role', level: 1 },
      permissions: []
    };
  ...
  async generateComplianceReport(
    ...
    const recommendations = await this.generateRecommendations(auditTrail, violations);
    const dataRetentionStatus = await this.getDataRetentionStatus();
    const encryptionStatus = await this.getEncryptionStatus();

    const complianceScore = this.calculateComplianceScore(auditTrail, violations);
  ...
  private sanitizeDetails(details: Record<string, any>, sensitivityLevel: string): Record<string, any> {
    ...
    if (sensitivityLevel === 'high' || sensitivityLevel === 'critical') {
      ...
      if (sanitized.demographics) {
        sanitized.demographics = this.anonymizeDemographics(sanitized.demographics);
      }
  ...
  private anonymizeDemographics(demographics: ParticipantDemographics): Partial<ParticipantDemographics> {
    return {
      age: demographics.age,
      gender: demographics.gender,
      ethnicity: demographics.ethnicity,
      primaryLanguage: demographics.primaryLanguage,
      // Remove more specific identifiers
      socioeconomicStatus: demographics.socioeconomicStatus,
      region: demographics.region ? 'REDACTED' : undefined
    };
  }
  ...
  private async detectViolations(auditTrail: AuditLogEntry[]): Promise<ComplianceViolation[]> {
    const violations: ComplianceViolation[] = [];

    // Check for unauthorized access patterns
    const failedLogins = auditTrail.filter(e => 
      e.action.type === 'login' && !e.success
    );
  ...
  private async generateRecommendations(
    ...
    const highSensitivityAccess = auditTrail.filter(e => 
      e.action.sensitivityLevel === 'high' || e.action.sensitivityLevel === 'critical'
    );
  ...
  private detectTimeGaps(auditTrail: AuditLogEntry[]): { start: Date; end: Date }[] {
    const gaps: { start: Date; end: Date }[] = [];
    const sortedEntries = auditTrail.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    ...
    for (let i = 1; i < sortedEntries.length; i++) {
      ...
      if (hoursDiff > 24) {
        gaps.push({
          start: sortedEntries[i - 1].timestamp,
          end: sortedEntries[i].timestamp
        });
      }
    }
...
export async function logBiasAnalysisAction(
  ...
  const auditAction: AuditAction = {
    type: action === 'analyze' ? 'create' : action === 'export' ? 'export' : 'read',
    category: 'bias-analysis',
    description: `${action} bias analysis for session ${sessionId}`,
    sensitivityLevel: details.alertLevel === 'critical' ? 'critical' : 'medium'
  };
...
export async function logDataExport(
  ...
  const auditAction: AuditAction = {
    ...
    sensitivityLevel: recordCount > 1000 ? 'high' : recordCount > 100 ? 'medium' : 'low'
  };
