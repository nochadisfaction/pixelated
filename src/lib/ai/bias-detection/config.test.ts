...
describe('BiasDetectionConfigManager', () => {
  ...
  describe('Production Readiness Check', () => {
    ...
    it('should identify missing production requirements', () => {
      ...
      expect(readiness.issues).toContain('JWT secret is missing or too short (minimum 32 characters)');
      expect(readiness.issues).toContain('Encryption key is missing or too short (minimum 32 characters)');
    });
    ...
    it('should warn about debug logging in production', () => {
      ...
      expect(readiness.ready).toBe(false);
      expect(readiness.issues).toContain('Debug logging enabled in production - may expose sensitive data');
    });
  });
