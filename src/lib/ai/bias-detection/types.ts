...
export interface TrainingScenario {
  scenarioId: string;
  type: 'depression' | 'anxiety' | 'trauma' | 'substance-abuse' | 'grief' | 'other';
...
export interface DemographicGroup {
  type: 'age' | 'gender' | 'ethnicity' | 'language' | 'socioeconomic' | 'education' | 'region';
  value: string;
  subgroups?: string[];
}
...
export interface Permission {
  resource: 'bias-analysis' | 'dashboard' | 'reports' | 'configuration' | 'user-management';
...
export interface ComplianceViolation {
  id: string;
  type: 'data-retention' | 'unauthorized-access' | 'missing-audit' | 'encryption-failure';
