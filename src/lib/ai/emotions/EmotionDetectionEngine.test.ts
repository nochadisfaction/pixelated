...
describe('EmotionDetectionEngine', () => {
  ...
  beforeEach(() => {
    ...
    (PerformanceLogger.getInstance as MockedFunction<typeof PerformanceLogger.getInstance>)
  ...
  describe('Single Emotion Analysis', () => {
    ...
    it('should handle empty text input', async () => {
      ...
      await expect(emotionEngine.analyzeEmotion(emptyData))
    ...
    it('should handle null or undefined text', async () => {
      ...
      await expect(emotionEngine.analyzeEmotion(nullData))
        .rejects.toThrow('Text input cannot be empty');
    });
    ...
    it('should handle provider errors gracefully', async () => {
      ...
      await expect(emotionEngine.analyzeEmotion(mockEmotionData))
    ...
    it('should filter emotions below confidence threshold', async () => {
      ...
      mockEmotionProvider.analyzeEmotion.mockResolvedValue(lowConfidenceAnalysis);

      const result = await emotionEngine.analyzeEmotion(mockEmotionData);

      // Should only include emotions above threshold (0.6)
      expect(Object.keys(result.emotions)).toEqual(['anxiety', 'trust', 'anticipation']);
    });
  ...
  describe('Batch Emotion Analysis', () => {
    ...
    it('should process batch emotions successfully', async () => {
      const batchResults = [
        mockEmotionAnalysis,
        { ...mockEmotionAnalysis, primaryEmotion: 'joy' as EmotionType, sessionId: 'session-124' },
        { ...mockEmotionAnalysis, primaryEmotion: 'anger' as EmotionType, sessionId: 'session-125' }
      ];
    ...
    it('should handle empty batch input', async () => {
      await expect(emotionEngine.batchAnalyzeEmotions([]))
        .rejects.toThrow('Batch data cannot be empty');
    });
    ...
    it('should handle batch processing errors', async () => {
      ...
      await expect(emotionEngine.batchAnalyzeEmotions(mockBatchData))
  ...
  describe('Performance Monitoring', () => {
    it('should log performance metrics', async () => {
      ...
      expect(mockPerformanceLogger.startTimer).toHaveBeenCalledWith('emotion_analysis');
    ...
    it('should track batch processing performance', async () => {
      ...
      expect(mockPerformanceLogger.startTimer).toHaveBeenCalledWith('batch_emotion_analysis');
      expect(mockPerformanceLogger.logMetric).toHaveBeenCalledWith(
  ...
  describe('Risk Factor Detection', () => {
    ...
    it('should provide appropriate recommendations for risk factors', async () => {
      ...
      expect(result.recommendations).toContain('Consider breathing exercises to manage anxiety');
      expect(result.recommendations).toContain('Practice presentation beforehand to build confidence');
    });
  });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle FHE service unavailability when secure processing is enabled', async () => {
      ...
      await expect(secureEngine.analyzeEmotion(mockEmotionData))
    ...
    it('should validate emotion data structure', async () => {
      ...
      await expect(emotionEngine.analyzeEmotion(invalidData))
        .rejects.toThrow();
    });
  });
  ...
  describe('Configuration and Options', () => {
    ...
    it('should handle different confidence thresholds', async () => {
      ...
      mockEmotionProvider.analyzeEmotion.mockResolvedValue(mixedConfidenceAnalysis);
