...
/**
 * Test MentalLLaMA-7B Model Integration
 *
 * This CLI tool allows testing the MentalLLaMA-chat-7B model integration
 ...
 *
 * Usage:
 *   node test-mental-llama-7b.js "I've been feeling really down lately"
 */
...
/**
 * Run a test with the MentalLLaMA-7B model
 * @param {string} text Text to analyze
 */
async function runTest(text) {
  try {
    console.log(`\n🧠 MentalLLaMA-7B Model Test`)
    console.log(`───────────────────────────`)
    ...
    if (!modelProvider) {
      ...
      console.error(`  - USE_MENTAL_LLAMA_7B_MODEL=true`)
      console.error(`  - MENTAL_LLAMA_7B_API_URL=<api_url>`)
      console.error(`  - MENTAL_LLAMA_7B_API_KEY=<api_key>`)
      console.error(`  - MENTAL_LLAMA_7B_MODEL_NAME=<model_name>`)
      process.exit(1)
    }

    // Verify we're using the 7B model
    const modelTier = modelProvider.getModelTier()
    if (modelTier !== '7B') {
      console.error(
        `❌ ERROR: Not using the 7B model. Current model tier: ${modelTier}`,
      )
      process.exit(1)
    }

    console.log(`✅ Connected to MentalLLaMA-chat-7B model successfully`)
...
function runInteractiveMode() {
  console.log(`\n🧠 MentalLLaMA-7B Model Interactive Test`)
  console.log(`────────────────────────────────────`)
