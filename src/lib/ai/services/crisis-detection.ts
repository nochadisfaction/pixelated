...
export class CrisisDetectionService {
  ...
  private static readonly CRISIS_KEYWORDS = {
    self_harm: [
      'self-harm', 'hurt myself', 'cutting', 'self-injury', 'self-mutilation',
      'harming myself', 'want to hurt myself'
    ],
    suicide: [
      'suicide', 'kill myself', 'end my life', 'want to die', 'suicidal',
      'not worth living', 'better off dead', 'suicide plan', 'take my own life'
    ],
    emergency: [
      'immediate danger', 'going to hurt', 'right now', 'tonight',
      'overdose', 'pills', 'weapon', 'bridge', 'plan to'
    ],
    severe_depression: [
      'hopeless', 'worthless', 'no point', 'give up', 'can\'t go on',
      'unbearable pain', 'too much pain', 'can\'t take it'
    ],
    moderate_concern: [
      'depressed', 'anxious', 'panic', 'overwhelmed', 'struggling',
      'difficult time', 'very sad', 'can\'t cope'
    ]
  }
  ...
  async detectCrisis(
    ...
    try {
      ...
      return {
        ...
        category: this.determineCrisisCategory(keywordAnalysis.indicators, aiAnalysis?.category),
        content: text,
        ...
        suggestedActions: this.generateSuggestedActions(finalScore, keywordAnalysis.indicators),
  ...
  private analyzeKeywords(text: string): { score: number; indicators: string[] } {
    const lowerText = text.toLowerCase()
    const indicators: string[] = []
    let score = 0

    // Check each category with different weights
    for (const [category, keywords] of Object.entries(CrisisDetectionService.CRISIS_KEYWORDS)) {
  ...
  private async performAIAnalysis(text: string): Promise<RiskAssessment> {
    try {
      ...
      const response = await this.aiService.createChatCompletion([
        { role: 'system', content: 'You are a mental health crisis detection AI. Provide accurate, helpful analysis.' },
  ...
  private determineCrisisCategory(keywords: string[], aiCategory?: string): string {
    if (aiCategory) return aiCategory

    // Determine category based on keywords
    if (keywords.some(k => CrisisDetectionService.CRISIS_KEYWORDS.emergency.includes(k))) {
      return 'emergency'
    }
    if (keywords.some(k => CrisisDetectionService.CRISIS_KEYWORDS.suicide.includes(k))) {
      return 'suicide_risk'
    }
    if (keywords.some(k => CrisisDetectionService.CRISIS_KEYWORDS.self_harm.includes(k))) {
      return 'self_harm_risk'
    }
    if (keywords.some(k => CrisisDetectionService.CRISIS_KEYWORDS.severe_depression.includes(k))) {
  ...
  private determineUrgency(score: number, indicators: string[]): 'low' | 'medium' | 'high' | 'immediate' {
  ...
  private generateSuggestedActions(score: number, indicators: string[]): string[] {
