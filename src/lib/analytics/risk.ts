...
export class RiskScoring {
  private static readonly BASE_RISK = 0.15
  private static readonly MAX_CACHE_AGE = 3600000 // 1 hour in milliseconds

  // Dynamic risk factors with scoring logic
  private static readonly RISK_FACTORS: RiskFactor[] = [
    {
   ...
   */
  static async calculateOverallRisk(
    ...
    try {
      ...
      // Check cache first
      const cacheKey = this.generateCacheKey(breaches)
      ...
      // Base risk level
      let overallScore = this.BASE_RISK
      const factorResults = []

      // Calculate individual factor scores
      for (const factor of this.RISK_FACTORS) {
        try {
      ...
      const recommendations = this.generateRecommendations(factorResults)

      // Calculate confidence score based on data quality
      const confidence = this.calculateConfidence(breaches, factorResults)
   ...
   */
  static async calculateDailyRisk(
    breaches: SecurityBreach[],
  ): Promise<RiskAssessment> {
    const assessment = await this.calculateOverallRisk(breaches)
   ...
   */
  static async getFactors(): Promise<RiskFactor[]> {
    return this.RISK_FACTORS.map((factor) => ({
      ...factor,
   ...
   */
  private static generateCacheKey(breaches: SecurityBreach[]): string {
