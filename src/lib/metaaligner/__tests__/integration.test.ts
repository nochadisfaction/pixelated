...
import type { AIMessage, AIService, AIServiceResponse } from '../../../ai/models/types';
  ...
  const mockService: Partial<AIService> = {
    createChatCompletion: vi.fn().mockResolvedValue({
      ...
      choices: [{
        ...
        message: {
          role: 'assistant',
          content: enhancedResponse || 'Enhanced response with better alignment to mental health objectives. This response shows improved empathy, accuracy, and safety considerations.',
          name: 'assistant'
        },
        finishReason: 'stop'
      }],
...
describe('MetaAligner Integration Tests', () => {
  ...
  describe('Integrated AI Service Workflow', () => {
    it('should automatically evaluate and enhance responses', async () => {
      ...
      const messages: AIMessage[] = [
        { role: 'user', content: 'I feel anxious and don\'t know what to do', name: 'user' }
      ];
    ...
    it('should handle conversation history in context detection', async () => {
      ...
      const messages: AIMessage[] = [
        ...
        { role: 'assistant', content: 'I understand you\'re feeling sad. Can you tell me more?', name: 'assistant' },
        { role: 'user', content: 'It\'s been going on for weeks now', name: 'user' }
      ];

      const response = await integratedService.createChatCompletion(messages);

      expect(response.alignment?.evaluation?.evaluationContext?.conversationHistory).toBeDefined();
      expect(response.alignment?.evaluation?.evaluationContext?.conversationHistory?.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('Objective Balancing and Prioritization', () => {
    it('should adapt objective weights based on context', async () => {
      ...
      const contexts = [
        ...
        { query: 'What is depression?', expectedContext: ContextType.EDUCATIONAL },
        { query: 'I need emotional support', expectedContext: ContextType.SUPPORT }
      ];
    ...
    it('should maintain objective balance across different response types', async () => {
      ...
      for (const response of responses) {
        const evaluation = await metaAligner.evaluateResponse({ response, context });
  ...
  describe('Enhancement Pipeline Integration', () => {
    ...
    it('should handle enhancement failures gracefully', async () => {
      ...
      await expect(metaAlignerWithFailingAI.enhanceResponse({
