...
import { MetaAlignerAPI, IntegratedAIService, type AlignmentIntegrationConfig } from './alignment-api';
...
describe('MetaAlignerAPI', () => {
  ...
  describe('evaluateResponse', () => {
    it('should evaluate a mental health response', async () => {
      const request = {
        response: 'I understand that you\'re feeling anxious. This is a common experience, and there are evidence-based strategies that can help.',
        context: {
    ...
    it('should evaluate all core objectives', async () => {
      ...
      expect(Object.keys(result.evaluation.objectiveResults)).toContain('correctness');
      expect(Object.keys(result.evaluation.objectiveResults)).toContain('empathy');
      expect(Object.keys(result.evaluation.objectiveResults)).toContain('safety');
      expect(Object.keys(result.evaluation.objectiveResults)).toContain('professionalism');
      expect(Object.keys(result.evaluation.objectiveResults)).toContain('informativeness');
    });
    ...
    it('should provide detailed objective explanations', async () => {
      ...
      for (const [objectiveId, objectiveResult] of Object.entries(result.evaluation.objectiveResults)) {
  ...
  describe('enhanceResponse', () => {
    beforeEach(() => {
      ...
      (mockAIService.createChatCompletion as any).mockResolvedValue({
        choices: [{
          message: {
            content: 'Enhanced response with better empathy and professionalism'
          }
        }],
        model: 'test-model',
        usage: {
    ...
    it('should throw error when AI service not configured', async () => {
      ...
      await expect(apiWithoutAI.enhanceResponse(enhancementRequest))
  ...
  describe('createIntegratedService', () => {
    it('should create an integrated AI service', () => {
      const integratedService = metaAligner.createIntegratedService(mockAIService as AIService);
...
describe('IntegratedAIService', () => {
  ...
  beforeEach(() => {
    ...
    integratedService = new IntegratedAIService(mockAIService as AIService, metaAligner);
  });
  ...
  describe('createChatCompletion', () => {
    beforeEach(() => {
      ...
      (mockAIService.createChatCompletion as any).mockResolvedValue({
        choices: [{
          message: {
            content: 'I understand you are going through a difficult time. Mental health challenges are common and treatable.'
          }
        }],
        model: 'test-model',
        usage: {
    ...
    it('should perform enhancement when response quality is low', async () => {
      ...
      (mockAIService.createChatCompletion as any)
        ...
        .mockResolvedValueOnce({
          choices: [{
            message: {
              content: 'I understand this is difficult for you. Here are some strategies that might help...' // Enhanced response
            }
          }],
          model: 'test-model'
        });
    ...
    it('should correctly detect different conversation contexts', async () => {
      ...
      for (const testCase of testCases) {
        ...
        expect(result.alignment?.evaluation.evaluationContext.detectedContext)
  ...
  describe('Integration with Real Conversation Flow', () => {
    it('should maintain conversation context across multiple exchanges', async () => {
      const conversationMessages: AIMessage[] = [
        { role: 'user', content: 'I have been feeling anxious lately', name: 'user' },
        { role: 'assistant', content: 'I understand anxiety can be challenging. What specific situations trigger your anxiety?', name: 'assistant' },
        { role: 'user', content: 'Mostly social situations and work meetings', name: 'user' }
      ];

      const result = await integratedService.createChatCompletion(conversationMessages);

      expect(result.alignment?.evaluation.evaluationContext.conversationHistory).toHaveLength(3);
      expect(result.alignment?.evaluation.evaluationContext.conversationHistory).toContain('I have been feeling anxious lately');
    });

    it('should handle streaming-like behavior with static responses', async () => {
      const messages: AIMessage[] = [
        { role: 'user', content: 'Help me understand my emotions', name: 'user' }
      ];
      ...
      expect(result1.alignment?.evaluation.evaluationContext.detectedContext).toBe(ContextType.EDUCATIONAL);
    });
  });

  describe('Error Handling and Edge Cases', () => {
    it('should handle AI service errors gracefully', async () => {
      (mockAIService.createChatCompletion as any).mockRejectedValue(new Error('Network error'));

      const messages: AIMessage[] = [
        { role: 'user', content: 'Test message', name: 'user' }
      ];

      await expect(integratedService.createChatCompletion(messages))
    ...
    it('should work with disabled enhancement', async () => {
      ...
      const result = await serviceWithoutEnhancement.createChatCompletion(messages);
