...
import { ObjectiveMetricsEngine, AlignmentMetrics } from '../core/objective-metrics';
...
export class MetaAlignerAPI {
  ...
  async evaluateResponse(request: EvaluationRequest): Promise<EvaluationResponse> {
    ...
    try {
      ...
      const overallScore = this.calculateOverallScore(objectiveResults, objectives);

      // Create evaluation result
      const evaluation: AlignmentEvaluationResult = {
        objectiveResults,
        overallScore,
        weights: objectives.reduce((weights, obj) => {
          weights[obj.id] = obj.weight;
          return weights;
        }, {} as Record<string, number>),
        normalizedScores: Object.fromEntries(
          Object.entries(objectiveResults).map(([id, result]) => [id, result.score])
        ),
      ...
      const metrics = this.metricsEngine.calculateAlignmentMetrics(evaluation, objectives);
      ...
      const needsEnhancement = overallScore < (this.config.enhancementThreshold || 0.7);
   ...
   */
  async enhanceResponse(request: EnhancementRequest): Promise<EnhancementResponse> {
    const { originalResponse, evaluationResult, context, targetObjectives } = request;
    ...
    try {
      ...
      const improvementAreas = this.identifyImprovementAreas(evaluationResult, targetObjectives);
      ...
      const enhancementApplied = enhancedEvaluation.evaluation.overallScore > evaluationResult.overallScore;
      ...
      logger.info('Response enhancement completed', {
        ...
        improvement: (enhancedEvaluation.evaluation.overallScore - evaluationResult.overallScore).toFixed(3),
        enhancementApplied,
        action: 'enhance_response_complete'
      });

      return {
        enhancedResponse: enhancementApplied ? enhancedResponse : originalResponse,
      ...
      return {
        ...
        improvementMetrics: this.metricsEngine.calculateAlignmentMetrics(evaluationResult, this.objectives),
   ...
   */
  detectContext(userQuery: string, conversationHistory?: AIMessage[]): AlignmentContext {
    ...
    const crisisKeywords = ['suicide', 'kill myself', 'end it all', 'hurt myself', 'crisis', 'emergency'];
    if (crisisKeywords.some(keyword => queryLower.includes(keyword))) {
      detectedContext = ContextType.CRISIS;
    }
    // Educational context
    else if (queryLower.includes('learn') || queryLower.includes('understand') || queryLower.includes('explain')) {
      detectedContext = ContextType.EDUCATIONAL;
    }
    // Support context
    else if (queryLower.includes('support') || queryLower.includes('help') || queryLower.includes('feeling')) {
      detectedContext = ContextType.SUPPORT;
    }
    // Clinical assessment
    else if (queryLower.includes('symptoms') || queryLower.includes('diagnosis') || queryLower.includes('treatment')) {
      detectedContext = ContextType.CLINICAL_ASSESSMENT;
    }
    // Informational
    else if (queryLower.includes('what is') || queryLower.includes('how does') || queryLower.includes('information')) {
  ...
  private evaluateCriterion(response: string, context: AlignmentContext, criterion: string): number {
    ...
    switch (criterion) {
      ...
      case 'evidence_based':
        return response.includes('research') || response.includes('studies') ? 0.9 : 0.7;
      case 'clinical_soundness':
        return !response.includes('diagnose') && responseLength > 100 ? 0.85 : 0.6;
  ...
  private hasContextualWords(response: string, _context: AlignmentContext): boolean {
    const supportiveWords = ['understand', 'validate', 'support', 'empathy', 'feel', 'experience'];
  ...
  private containsHarmfulContent(response: string): boolean {
    const harmfulPatterns = ['you should hurt', 'kill yourself', 'give up', 'hopeless case'];
    const responseLower = response.toLowerCase();
    return harmfulPatterns.some(pattern => responseLower.includes(pattern));
  }

  private calculateConfidence(response: string, context: AlignmentContext, _objective: ObjectiveDefinition): number {
    ...
    const contextAlignment = context.detectedContext !== ContextType.GENERAL ? 0.9 : 0.7;
    return (responseQuality + contextAlignment) / 2;
  }

  private calculateOverallScore(objectiveResults: Record<string, ObjectiveEvaluationResult>, objectives: ObjectiveDefinition[]): number {
    let totalScore = 0;
    let totalWeight = 0;
  ...
  private generateRecommendations(evaluation: AlignmentEvaluationResult, metrics: AlignmentMetrics): string[] {
    const recommendations: string[] = [];
    
    // Check each objective for specific recommendations
    for (const [objectiveId, result] of Object.entries(evaluation.objectiveResults)) {
      if (result.score < 0.7) {
        const objective = this.objectives.find(o => o.id === objectiveId);
        if (objective) {
          recommendations.push(`Improve ${objective.name}: ${objective.description}`);
        }
      }
    }
    ...
    if (metrics.balanceScore < 0.7) {
      recommendations.push('Focus on balancing objectives to avoid over-optimization in specific areas.');
    }

    // Add context-specific recommendations
    if (metrics.contextualAlignment < 0.8) {
      recommendations.push('Ensure response better aligns with the detected conversation context.');
    }

    return recommendations;
  }

  private identifyImprovementAreas(evaluation: AlignmentEvaluationResult, targetObjectives?: string[]): string[] {
    const areas: string[] = [];
  ...
  private createEnhancementPrompt(_originalResponse: string, context: AlignmentContext, improvementAreas: string[], evaluation: AlignmentEvaluationResult): string {
...
Please enhance the response to better meet these objectives while maintaining its core message and helpfulness. Focus particularly on the identified improvement areas.`;
  ...
  private generateEnhancementExplanation(original: AlignmentEvaluationResult, enhanced: AlignmentEvaluationResult, improvementAreas: string[]): string {
    const overallImprovement = (enhanced.overallScore - original.overallScore) * 100;
    ...
    for (const [objectiveId, enhancedResult] of Object.entries(enhanced.objectiveResults)) {
      const originalResult = original.objectiveResults[objectiveId];
      if (originalResult && enhancedResult.score > originalResult.score) {
        const objective = this.objectives.find(o => o.id === objectiveId);
        if (objective) {
          const improvement = (enhancedResult.score - originalResult.score) * 100;
...
export class IntegratedAIService {
  ...
  async createChatCompletion(messages: AIMessage[], options?: AIStreamOptions): Promise<IntegratedResponse> {
    // Get the base response
    const baseResponse = await this.baseService.createChatCompletion(messages, options);
    ...
    const context = this.metaAligner.detectContext(lastUserMessage.content, messages);
    ...
    if (evaluation.needsEnhancement && this.metaAligner['config'].enableResponseEnhancement) {
