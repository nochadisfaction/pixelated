...
const createMockEvaluationFunction = (returnValue: number = 0.8) => 
  (response: string, context: AlignmentContext): number => returnValue;
...
describe('Objective Interfaces', () => {
  describe('StandardObjectiveBuilder', () => {
    ...
    test('should fail validation with invalid weight range', () => {
      ...
      const validation = builder.validate();
      expect(validation.isValid).toBe(false);
      expect(validation.errors.some(e => e.code === 'INVALID_WEIGHT_RANGE')).toBe(true);
    });
    ...
    test('should fail validation when criteria weights do not sum to 1', () => {
      ...
      const validation = builder.validate();
      expect(validation.isValid).toBe(false);
      expect(validation.errors.some(e => e.code === 'INVALID_CRITERIA_WEIGHTS')).toBe(true);
    });
    ...
    test('should show warning when no criteria are defined', () => {
      ...
      const validation = builder.validate();
      expect(validation.warnings.some(w => w.code === 'NO_CRITERIA')).toBe(true);
    });
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle multiple criteria addition', () => {
      const builder = new StandardObjectiveBuilder();
      const criterion1 = ObjectiveFactory.createCriterion('c1', 'Criterion 1', 0.3);
      const criterion2 = ObjectiveFactory.createCriterion('c2', 'Criterion 2', 0.7);
