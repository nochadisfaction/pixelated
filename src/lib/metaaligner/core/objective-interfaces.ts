...
export interface ObjectiveBuilder {
  ...
  setEvaluationFunction(fn: (response: string, context: AlignmentContext) => number): ObjectiveBuilder;
...
export class StandardObjectiveRegistry implements ObjectiveRegistry {
  ...
  register(objective: ObjectiveDefinition): void {
    ...
    if (!validation.isValid) {
      throw new Error(`Cannot register invalid objective: ${validation.errors.map(e => e.message).join(', ')}`);
    }
