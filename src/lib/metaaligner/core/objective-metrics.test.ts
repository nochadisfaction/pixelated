...
describe('ObjectiveMetricsEngine', () => {
    ...
    mockObjectives = [
      {
        ...
        weight: 0.4,
        criteria: [
          { criterion: 'accuracy', description: 'How accurate the response is', weight: 0.6 },
          { criterion: 'completeness', description: 'How complete the response is', weight: 0.4 }
        ],
      ...
      },
      {
        ...
        weight: 0.3,
        criteria: [
          { criterion: 'emotional_recognition', description: 'Ability to recognize emotions', weight: 0.5 },
          { criterion: 'supportive_language', description: 'Use of supportive language', weight: 0.5 }
        ],
      ...
      },
      {
        ...
        weight: 0.3,
        criteria: [
          { criterion: 'harm_prevention', description: 'Prevention of potential harm', weight: 0.7 },
          { criterion: 'risk_assessment', description: 'Assessment of risks', weight: 0.3 }
        ],
  ...
  describe('calculateObjectiveMetrics', () => {
    test('should calculate basic objective metrics correctly', () => {
      ...
      const metrics = engine.calculateObjectiveMetrics(objective, evaluationResult, mockContext);
    ...
    test('should calculate criteria breakdown correctly', () => {
      ...
      const accuracyCriteria = metrics.criteriaBreakdown.find(c => c.criterion === 'accuracy');
  ...
  describe('calculateAlignmentMetrics', () => {
    test('should calculate comprehensive alignment metrics', () => {
      const metrics = engine.calculateAlignmentMetrics(mockEvaluationResult, mockObjectives);
  ...
  describe('addEvaluation and history tracking', () => {
    test('should add evaluation to history', () => {
      ...
      expect(updatedBenchmarks.length).toBeGreaterThanOrEqual(initialBenchmarks.length);
    });
    ...
    test('should maintain history window size', () => {
      ...
      expect(smallEngine['evaluationHistory'].evaluations.length).toBeLessThanOrEqual(6);
    });
  });
  ...
  describe('contextual fit calculation', () => {
    test('should calculate higher fit for safety in crisis context', () => {
      ...
      const metrics = engine.calculateObjectiveMetrics(safetyObjective, evaluationResult, crisisContext);
    ...
    test('should calculate higher fit for correctness in clinical context', () => {
      ...
      const evaluationResult = mockEvaluationResult.objectiveResults.correctness;
      const metrics = engine.calculateObjectiveMetrics(correctnessObjective, evaluationResult, clinicalContext);
  ...
  describe('benchmarks', () => {
    test('should track benchmark achievement rates', () => {
      ...
      const highPerformanceBenchmark = benchmarks.find(b => b.name === 'High Performance');
  ...
  describe('edge cases', () => {
    ...
    test('should handle missing criteria scores gracefully', () => {
      const objectiveWithMissingCriteria = {
        ...mockObjectives[0],
        criteria: [
          { criterion: 'missing_criterion', description: 'A criterion with missing score', weight: 1.0 }
        ]
      };
...
describe('DEFAULT_METRICS_CONFIG', () => {
  test('should have reasonable default values', () => {
    ...
    expect(DEFAULT_METRICS_CONFIG.benchmarkThresholds.excellent).toBeGreaterThan(
      DEFAULT_METRICS_CONFIG.benchmarkThresholds.good
    );
