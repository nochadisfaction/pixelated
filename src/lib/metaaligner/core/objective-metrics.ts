...
export class ObjectiveMetricsEngine {
  ...
  addEvaluation(
    ...
    if (this.evaluationHistory.evaluations.length > this.config.historyWindow * 2) {
      this.evaluationHistory.evaluations = this.evaluationHistory.evaluations
  ...
  private calculateReliability(objectiveId: string): number {
    ...
    const scores = recentEvaluations.map(e => 
      e.evaluation.objectiveResults[objectiveId]?.score || 0
    );
  ...
  private calculateImprovement(objectiveId: string, currentScore: number): number {
    ...
    if (baseline === undefined) {
      ...
      const firstEvaluation = this.evaluationHistory.evaluations
        .find(e => e.evaluation.objectiveResults[objectiveId]);
      
      if (firstEvaluation) {
        const firstScore = firstEvaluation.evaluation.objectiveResults[objectiveId].score;
  ...
  private calculateOverallContextualAlignment(objectiveMetrics: Record<string, ObjectiveMetrics>): number {
  ...
  private calculateQualityIndicators(objectiveMetrics: Record<string, ObjectiveMetrics>): QualityIndicators {
  ...
  private generatePerformanceProfile(objectiveMetrics: Record<string, ObjectiveMetrics>): PerformanceProfile {
    const sortedMetrics = Object.entries(objectiveMetrics)
  ...
  private getHistoricalConfidence(objectiveId: string): number {
    ...
    const confidenceScores = recentEvaluations.map(e => 
      e.evaluation.objectiveResults[objectiveId]?.confidence || 0.5
    );
  ...
  private getLatestOverallScore(): number {
    const latest = this.evaluationHistory.evaluations[this.evaluationHistory.evaluations.length - 1];
