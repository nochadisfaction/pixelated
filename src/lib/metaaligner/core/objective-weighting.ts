...
export class ObjectiveWeightingEngine {
  ...
  calculateWeights(
    ...
    switch (this.adjustmentParams.strategy) {
      ...
      case WeightingStrategy.CONTEXTUAL:
        const contextResult = this.applyContextualWeighting(baseWeights, context);
      ...
      case WeightingStrategy.PERFORMANCE_BASED:
        adjustedWeights = this.applyPerformanceBasedWeighting(baseWeights, objectives);
        break;
  ...
  private getBaseWeights(
    ...
    for (const objective of objectives) {
      weights[objective.id] = config.objectives[objective.id] ?? objective.weight;
    }
  ...
  private applyContextualWeighting(
    ...
    context: AlignmentContext
  ): { weights: Record<string, number>; factors: Record<string, number>; contextFactors: string[] } {
    ...
    const multipliers = this.contextMultipliers[contextType] || this.contextMultipliers.default;

    for (const [objectiveId, baseWeight] of Object.entries(baseWeights)) {
      const multiplier = multipliers[objectiveId] || 1.0;
      const adjustmentFactor = 1.0 + (multiplier - 1.0) * this.adjustmentParams.contextSensitivity;
  ...
  private applyAdaptiveWeighting(
    ...
    for (const [objectiveId, baseWeight] of Object.entries(baseWeights)) {
      ...
      if (performanceHistory.length > 0) {
        ...
        const performanceAdjustment = (1.0 - avgPerformance) * this.adjustmentParams.adaptationRate;
        weights[objectiveId] = Math.max(0.01, baseWeight + performanceAdjustment);
      }
    }
  ...
  private applyPerformanceBasedWeighting(
    ...
    for (const objective of objectives) {
      ...
      if (history.length >= this.adjustmentParams.performanceWindow) {
        const recentHistory = history.slice(-this.adjustmentParams.performanceWindow);
        const avgScore = recentHistory.reduce((sum, score) => sum + score, 0) / recentHistory.length;
  ...
  private determineContextType(context: AlignmentContext): keyof ContextWeightMultipliers {
  ...
  private normalizeWeights(weights: Record<string, number>): Record<string, number> {
    ...
    if (total === 0) {
      ...
      return Object.keys(weights).reduce((normalized, id) => {
  ...
  private calculateWeightVariance(weights: Record<string, number>): number {
    ...
    const variance = values.reduce((sum, weight) => sum + Math.pow(weight - mean, 2), 0) / values.length;
  ...
  private recordWeightAdjustments(
    ...
    for (const [objectiveId, adjustedWeight] of Object.entries(adjustedWeights)) {
  ...
  updatePerformanceHistory(objectiveId: string, score: number): void {
    ...
    if (history.length > this.adjustmentParams.performanceWindow * 2) {
      history.splice(0, history.length - this.adjustmentParams.performanceWindow);
    }
  }
}
...
export class ObjectiveBalancer {
  ...
  static balance(
    ...
    return {
      ...
      evaluationContext: evaluationResults[Object.keys(evaluationResults)[0]]?.metadata?.contextFactors[0] as any || {} as AlignmentContext,
      timestamp: new Date()
    };
  }

  private static extractScores(evaluationResults: Record<string, ObjectiveEvaluationResult>): Record<string, number> {
  ...
  private static minMaxNormalize(scores: Record<string, number>): Record<string, number> {
  ...
  private static zScoreNormalize(scores: Record<string, number>): Record<string, number> {
    const values = Object.values(scores);
  ...
  private static sigmoidNormalize(scores: Record<string, number>): Record<string, number> {
    const normalized: Record<string, number> = {};
  ...
  private static weightedAverage(scores: Record<string, number>, weights: Record<string, number>): number {
    let weightedSum = 0;
    let totalWeight = 0;
  ...
  private static weightedSum(scores: Record<string, number>, weights: Record<string, number>): number {
    let sum = 0;
  ...
  private static harmonicMean(scores: Record<string, number>, weights: Record<string, number>): number {
    let weightedSum = 0;
    let totalWeight = 0;
  ...
  private static geometricMean(scores: Record<string, number>, weights: Record<string, number>): number {
    let logSum = 0;
    let totalWeight = 0;
