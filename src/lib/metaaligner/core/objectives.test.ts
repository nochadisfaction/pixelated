...
describe('Core Mental Health Objectives', () => {
  describe('Objective Definitions', () => {
    ...
    test('should contain all expected objective IDs', () => {
      const expectedIds = ['correctness', 'informativeness', 'professionalism', 'empathy', 'safety'];
    ...
    test('each objective should have valid criteria', () => {
      CORE_MENTAL_HEALTH_OBJECTIVES.forEach(objective => {
        ...
        const totalCriteriaWeight = objective.criteria.reduce((sum, criterion) => sum + criterion.weight, 0);
  ...
  describe('Evaluation Functions', () => {
    ...
    test('informativeness evaluation should work', () => {
      const informativeness = getObjectiveById('informativeness')!;
      const score = informativeness.evaluationFunction(mockResponse, mockContext);
      expect(score).toBeGreaterThan(0.5);
    });

    test('professionalism evaluation should work', () => {
      const professionalism = getObjectiveById('professionalism')!;
      const score = professionalism.evaluationFunction(mockResponse, mockContext);
      expect(score).toBeGreaterThan(0.5);
    });
    ...
    test('should accept context with optional properties', () => {
      const context: AlignmentContext = {
        ...
        userProfile: {
          ...
          preferences: { communicationStyle: 'direct', levelOfDetail: 'detailed' },
          mentalHealthHistory: { conditions: ['anxiety'], treatmentHistory: true }
        },
      ...
      expect(context.userProfile?.preferences?.communicationStyle).toBe('direct');
      expect(context.userProfile?.mentalHealthHistory?.conditions).toContain('anxiety');
