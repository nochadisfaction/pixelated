...
import type { EducationalRecognizerConfig, EducationalContextResult } from './educational-context-recognizer';
  ...
  describe('recognizeEducationalContext', () => {
    describe('pattern-based recognition', () => {
      ...
      it('should identify topic areas correctly', async () => {
        const topicTests = [
          ...
          { query: 'How does anxiety affect the brain?', expectedTopic: TopicArea.ANXIETY },
          { query: 'What is CBT therapy?', expectedTopic: TopicArea.THERAPY },
          { query: 'How do antidepressants work?', expectedTopic: TopicArea.MEDICATION }
        ];

        for (const test of topicTests) {
          const result = await recognizer.recognizeEducationalContext(test.query);
    ...
    describe('AI-powered analysis', () => {
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                learningObjectives: ['Understand what depression is', 'Learn basic symptoms'],
                recommendedResources: ['educational_videos', 'infographics'],
                priorKnowledgeRequired: [],
                metadata: {
                  conceptualDepth: 0.3,
                  practicalApplications: ['self-awareness', 'recognition in others'],
      ...
      it('should include user profile context in AI analysis', async () => {
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                recommendedResources: ['scientific_articles', 'interactive_tools'],
        ...
        expect(result.recommendedResources).toContain(ResourceType.SCIENTIFIC_ARTICLES);
        
        // Verify that user context was passed to AI
        const callArgs = (mockAIService.createChatCompletion as Mock).mock.calls[0];
        const systemMessage = callArgs[0][0].content;
        expect(systemMessage).toContain('Education Level: graduate');
        expect(systemMessage).toContain('Prior Mental Health Knowledge: intermediate');
      });
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                priorKnowledgeRequired: [],
                metadata: { conceptualDepth: 0.9, practicalApplications: [], relatedTopics: [] }
              })
            }
          }]
        };
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                recommendedResources: ['educational_videos'],
                priorKnowledgeRequired: [],
                metadata: { conceptualDepth: 0.5, practicalApplications: [], relatedTopics: [] }
              })
            }
          }]
        };
