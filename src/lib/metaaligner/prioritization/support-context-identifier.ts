...
export class SupportContextIdentifier {
  ...
  async identifySupportContext(
    ...
    try {
      ...
      try {
        ...
        const aiResult = await this.performAIAnalysis(userQuery, conversationHistory, userEmotionalProfile);
  ...
  async identifyBatch(
    ...
    return Promise.all(
      queries.map(({ query, conversationHistory, userEmotionalProfile }) =>
        this.identifySupportContext(query, conversationHistory, userEmotionalProfile)
      )
    );
  }
   ...
   */
  private performPatternBasedIdentification(userQuery: string): SupportContextResult {
    const query = userQuery.toLowerCase();
    let bestEmotionalMatch = { state: EmotionalState.MIXED_EMOTIONS, confidence: 0 };
    let bestSupportMatch = { type: SupportType.EMOTIONAL_VALIDATION, confidence: 0 };
    let copingCapacity: 'high' | 'medium' | 'low' = 'medium';

    // Identify emotional state with priority scoring
    const emotionalMatches: Array<{state: EmotionalState, confidence: number, priority: number}> = [];
    ...
    for (const [emotion, patterns] of Object.entries(this.emotionalPatterns)) {
      for (const pattern of patterns) {
        if (pattern.test(query)) {
          ...
          emotionalMatches.push({ state: emotion as EmotionalState, confidence, priority });
        }
      }
    }
    ...
    if (emotionalMatches.length > 0) {
      emotionalMatches.sort((a, b) => b.priority - a.priority || b.confidence - a.confidence);
      const firstMatch = emotionalMatches[0];
      if (firstMatch) {
        bestEmotionalMatch = { state: firstMatch.state, confidence: firstMatch.confidence };
      }
    }

    // Identify support type with priority scoring
    const supportMatches: Array<{type: SupportType, confidence: number, priority: number}> = [];
    ...
    for (const [support, patterns] of Object.entries(this.supportPatterns)) {
      for (const pattern of patterns) {
        if (pattern.test(query)) {
          ...
          supportMatches.push({ type: support as SupportType, confidence, priority });
        }
      }
    }
    ...
    if (supportMatches.length > 0) {
      supportMatches.sort((a, b) => b.priority - a.priority || b.confidence - a.confidence);
      const firstMatch = supportMatches[0];
      if (firstMatch) {
        bestSupportMatch = { type: firstMatch.type, confidence: firstMatch.confidence };
      }
    }
    ...
    const overallConfidence = Math.max(bestEmotionalMatch.confidence, bestSupportMatch.confidence);
    const isSupport = hasEmotionalContent || hasSupportLanguage;
    const emotionalIntensity = this.calculateEmotionalIntensity(query, bestEmotionalMatch.state);
    const urgency = this.determineUrgency(emotionalIntensity, copingCapacity);
    let recommendedApproach = this.mapEmotionalStateToApproach(bestEmotionalMatch.state);
    ...
    if (this.adaptToEmotionalState && emotionalIntensity > 0.7) {
      ...
      } else if (recommendedApproach === RecommendedApproach.COGNITIVE_REFRAMING) {
    ...
    return {
      ...
      metadata: {
        ...
        immediateNeeds: this.extractImmediateNeeds(query, bestSupportMatch.type)
      }
    };
  }
...
Consider this context in your assessment.`;
    ...
    try {
      ...
      return {
        ...
        supportNeeds: Array.isArray(parsed.supportNeeds) 
          ? parsed.supportNeeds.map((n: any) => this.validateSupportNeed(n)).filter(Boolean)
          : [],
        recommendedApproach: this.validateRecommendedApproach(parsed.recommendedApproach),
        emotionalIntensity: Math.max(0, Math.min(1, parsed.emotionalIntensity || 0.5)),
        metadata: {
          emotionalIndicators: Array.isArray(parsed.metadata?.emotionalIndicators) 
            ? parsed.metadata.emotionalIndicators 
            : [],
          copingCapacity: this.validateCopingCapacity(parsed.metadata?.copingCapacity),
          socialSupport: this.validateSocialSupport(parsed.metadata?.socialSupport),
  ...
  private combineResults(
    ...
    const combinedConfidence = (aiResult.confidence * 0.75) + (patternResult.confidence * 0.25);
    ...
    return {
      ...
      emotionalIntensity: Math.max(patternResult.emotionalIntensity, aiResult.emotionalIntensity),
      // Combine metadata
      metadata: {
        ...aiResult.metadata,
        emotionalIndicators: [
  ...
  private validateRecommendedApproach(approach: string): RecommendedApproach {
    return Object.values(RecommendedApproach).includes(approach as RecommendedApproach)
  ...
  private validateSocialSupport(support: string): 'strong' | 'moderate' | 'limited' | 'unknown' {
    return ['strong', 'moderate', 'limited', 'unknown'].includes(support)
      ? support as 'strong' | 'moderate' | 'limited' | 'unknown'
      : 'unknown';
  }

  private calculateEmotionalIntensity(query: string, emotionalState: EmotionalState): number {
    ...
    const highIntensityStates = [EmotionalState.HOPELESSNESS, EmotionalState.OVERWHELM, EmotionalState.HELPLESSNESS];
    const mediumIntensityStates = [EmotionalState.SADNESS, EmotionalState.ANXIETY, EmotionalState.ANGER];
    ...
    const crisisKeywords = /\b(?:suicidal|suicide|kill myself|end it all|can't go on|give up)\b/i;
  ...
  private determineUrgency(emotionalIntensity: number, copingCapacity: 'high' | 'medium' | 'low'): 'low' | 'medium' | 'high' {
  ...
  private mapSupportTypeToNeeds(supportType: SupportType): SupportNeed[] {
    const needsMap: Record<SupportType, SupportNeed[]> = {
      [SupportType.EMOTIONAL_VALIDATION]: [SupportNeed.VALIDATION, SupportNeed.EMOTIONAL_REGULATION],
      [SupportType.COPING_ASSISTANCE]: [SupportNeed.SKILL_BUILDING, SupportNeed.PRACTICAL_ADVICE],
      [SupportType.ENCOURAGEMENT]: [SupportNeed.HOPE_RESTORATION, SupportNeed.PERSPECTIVE_TAKING],
      [SupportType.ACTIVE_LISTENING]: [SupportNeed.VALIDATION],
      [SupportType.PRACTICAL_GUIDANCE]: [SupportNeed.PRACTICAL_ADVICE, SupportNeed.RESOURCE_CONNECTION],
      [SupportType.GRIEF_SUPPORT]: [SupportNeed.VALIDATION, SupportNeed.MEANING_MAKING],
      [SupportType.RELATIONSHIP_SUPPORT]: [SupportNeed.RELATIONSHIP_REPAIR, SupportNeed.PERSPECTIVE_TAKING],
      [SupportType.STRESS_MANAGEMENT]: [SupportNeed.EMOTIONAL_REGULATION, SupportNeed.SKILL_BUILDING],
      [SupportType.IDENTITY_SUPPORT]: [SupportNeed.MEANING_MAKING, SupportNeed.PERSPECTIVE_TAKING],
      [SupportType.TRANSITION_SUPPORT]: [SupportNeed.PRACTICAL_ADVICE, SupportNeed.EMOTIONAL_REGULATION],
      [SupportType.TRAUMA_SUPPORT]: [SupportNeed.SAFETY_PLANNING, SupportNeed.VALIDATION],
      [SupportType.DAILY_FUNCTIONING]: [SupportNeed.SKILL_BUILDING, SupportNeed.RESOURCE_CONNECTION]
    };

    return needsMap[supportType] || [SupportNeed.VALIDATION];
  }

  private mapEmotionalStateToApproach(emotionalState: EmotionalState): RecommendedApproach {
  ...
  private extractEmotionalIndicators(_query: string): string[] {
    ...
    const emotionalWords = [
      'sad', 'happy', 'angry', 'scared', 'worried', 'excited', 'frustrated',
      'hopeless', 'helpless', 'overwhelmed', 'anxious', 'depressed', 'lonely',
      'guilty', 'ashamed', 'confused', 'hurt', 'disappointed', 'stressed'
    ];
  ...
  private extractImmediateNeeds(_query: string, supportType: SupportType): string[] {
    const needsMap: Record<SupportType, string[]> = {
      [SupportType.EMOTIONAL_VALIDATION]: ['empathy', 'understanding', 'acknowledgment'],
      [SupportType.COPING_ASSISTANCE]: ['coping strategies', 'management techniques', 'practical tools'],
      [SupportType.ENCOURAGEMENT]: ['hope', 'motivation', 'confidence building'],
      [SupportType.ACTIVE_LISTENING]: ['non-judgmental presence', 'safe space', 'patient listening'],
      [SupportType.PRACTICAL_GUIDANCE]: ['concrete steps', 'actionable advice', 'clear direction'],
      [SupportType.GRIEF_SUPPORT]: ['grief processing', 'loss acknowledgment', 'healing space'],
      [SupportType.RELATIONSHIP_SUPPORT]: ['relationship guidance', 'communication help', 'boundary setting'],
      [SupportType.STRESS_MANAGEMENT]: ['stress relief', 'relaxation techniques', 'workload management'],
      [SupportType.IDENTITY_SUPPORT]: ['self-exploration', 'identity clarification', 'purpose finding'],
      [SupportType.TRANSITION_SUPPORT]: ['change management', 'adaptation strategies', 'stability'],
      [SupportType.TRAUMA_SUPPORT]: ['safety', 'stabilization', 'trauma processing'],
      [SupportType.DAILY_FUNCTIONING]: ['routine establishment', 'basic self-care', 'functional support']
    };
  ...
  private getLongerTermStrategies(result: SupportContextResult): string[] {
    const strategies: Record<SupportType, string[]> = {
      [SupportType.EMOTIONAL_VALIDATION]: ['Build emotional awareness and self-understanding', 'Develop self-compassion practices'],
      [SupportType.COPING_ASSISTANCE]: ['Learn diverse coping strategies and practice regularly', 'Build resilience skills and stress tolerance'],
      [SupportType.ENCOURAGEMENT]: ['Develop hope and optimism through positive psychology', 'Build self-efficacy and confidence'],
      [SupportType.PRACTICAL_GUIDANCE]: ['Develop problem-solving skills and decision-making', 'Practice implementing structured approaches'],
      [SupportType.STRESS_MANAGEMENT]: ['Implement comprehensive stress management plan', 'Build relaxation and mindfulness skills'],
      [SupportType.RELATIONSHIP_SUPPORT]: ['Improve communication skills and emotional intelligence', 'Build healthy boundaries and relationship patterns'],
      [SupportType.TRAUMA_SUPPORT]: ['Process trauma with qualified professional support', 'Build safety, trust, and healing practices'],
      [SupportType.GRIEF_SUPPORT]: ['Work through grief stages with professional guidance', 'Build healthy coping and meaning-making practices'],
      [SupportType.ACTIVE_LISTENING]: ['Develop self-reflection and emotional processing skills', 'Build support networks and connection'],
      [SupportType.IDENTITY_SUPPORT]: ['Explore identity and values through self-discovery', 'Build authentic self-expression and purpose'],
      [SupportType.TRANSITION_SUPPORT]: ['Develop change management and adaptation skills', 'Build flexibility and resilience for transitions'],
      [SupportType.DAILY_FUNCTIONING]: ['Establish sustainable routines and self-care practices', 'Build functional skills and support systems']
    };

    return strategies[result.supportType] || ['Continue building emotional awareness and coping skills', 'Develop healthy patterns and support networks'];
  }
  ...
  private getRelevantResources(result: SupportContextResult): string[] {
    ...
    const resources: Record<SupportType, string[]> = {
      [SupportType.EMOTIONAL_VALIDATION]: ['Support groups and peer counseling', 'Mental health therapy', 'Journaling and reflection apps'],
      [SupportType.COPING_ASSISTANCE]: ['Coping skills workshops and classes', 'Self-help resources and books', 'Cognitive behavioral therapy'],
      [SupportType.ENCOURAGEMENT]: ['Inspirational content and success stories', 'Mentoring and coaching programs', 'Positive psychology resources'],
      [SupportType.PRACTICAL_GUIDANCE]: ['Life coaching and guidance counseling', 'Problem-solving workshops', 'Decision-making resources'],
      [SupportType.STRESS_MANAGEMENT]: ['Relaxation apps and mindfulness programs', 'Stress management courses', 'Meditation and breathing techniques'],
      [SupportType.GRIEF_SUPPORT]: ['Grief counseling and bereavement support', 'Grief support groups', 'Hospice and palliative care resources'],
      [SupportType.TRAUMA_SUPPORT]: ['Trauma-informed therapy and EMDR', 'Trauma support groups', 'PTSD treatment programs'],
      [SupportType.RELATIONSHIP_SUPPORT]: ['Couples and family therapy', 'Communication skills workshops', 'Relationship coaching'],
      [SupportType.ACTIVE_LISTENING]: ['Peer support programs and counseling', 'Active listening groups', 'Emotional processing workshops'],
      [SupportType.IDENTITY_SUPPORT]: ['Identity exploration therapy', 'Values clarification workshops', 'Self-discovery programs'],
      [SupportType.TRANSITION_SUPPORT]: ['Life transition counseling', 'Change management resources', 'Adaptation support groups'],
      [SupportType.DAILY_FUNCTIONING]: ['Occupational therapy services', 'Life skills training programs', 'Daily living support groups']
    };

    return resources[result.supportType] || ['Mental health counseling and therapy', 'Support communities and groups'];
  }
  ...
  private determineResponseStyle(result: SupportContextResult): {
    ...
    let approach: 'validating' | 'solution-focused' | 'exploratory' | 'stabilizing' = 'validating';
 ...
 */
export function createSupportContextIdentifier(config: SupportIdentifierConfig): SupportContextIdentifier {
 ...
 */
export function getDefaultSupportIdentifierConfig(aiService: AIService): SupportIdentifierConfig {
  return {
    aiService,
