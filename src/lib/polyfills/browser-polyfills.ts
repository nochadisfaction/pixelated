...
export const crypto = {
  ...
  createHash: (algorithm: string) => {
    console.warn('crypto.createHash is not fully supported in browser environment');
    return {
      update: (data: string) => ({
        digest: (encoding: string) => `browser-polyfill-hash-${data.substring(0, 8)}`
      })
    };
  },
  
  // Add subtle crypto API for modern browsers
  subtle: typeof window !== 'undefined' && window.crypto ? window.crypto.subtle : {
...
export const fs = {
  promises: {
    readFile: async (path: string, _options?: unknown) => {
      console.warn(`fs.promises.readFile called with path: ${path} - not supported in browser`);
    ...
    writeFile: async (path: string, _data: unknown, _options?: unknown) => {
      console.warn(`fs.promises.writeFile called with path: ${path} - not supported in browser`);
      throw new Error('fs.promises.writeFile is not supported in browser environment');
    },
    mkdir: async (path: string, _options?: unknown) => {
      console.warn(`fs.promises.mkdir called with path: ${path} - not supported in browser`);
      throw new Error('fs.promises.mkdir is not supported in browser environment');
    },
    stat: async (path: string) => {
      console.warn(`fs.promises.stat called with path: ${path} - not supported in browser`);
      throw new Error('fs.promises.stat is not supported in browser environment');
    },
    access: async (path: string, mode?: number) => {
      console.warn(`fs.promises.access called with path: ${path} - not supported in browser`);
      throw new Error('fs.promises.access is not supported in browser environment');
    }
  },
  readFileSync: (path: string, _options?: unknown) => {
    console.warn(`fs.readFileSync called with path: ${path} - not supported in browser`);
    throw new Error('fs.readFileSync is not supported in browser environment');
  },
  existsSync: (path: string) => {
    console.warn(`fs.existsSync called with path: ${path} - not supported in browser`);
    return false;
  }
};
...
export const child_process = {
  spawn: (command: string, _args?: string[], _options?: unknown) => {
    console.warn(`child_process.spawn called with command: ${command} - not supported in browser`);
    return {
  ...
  exec: (command: string, _options?: unknown, callback?: (...args: unknown[]) => void) => {
    console.warn(`child_process.exec called with command: ${command} - not supported in browser`);
    if (callback) {
      callback(new Error('child_process.exec is not supported in browser environment'), '', '');
    }
    throw new Error('child_process.exec is not supported in browser environment');
  },
  execSync: (command: string, _options?: unknown) => {
    console.warn(`child_process.execSync called with command: ${command} - not supported in browser`);
    throw new Error('child_process.execSync is not supported in browser environment');
  }
};
...
export const stream = {
  Readable: class {
    on(event: string, listener: (...args: unknown[]) => void) { return this; }
    pipe(destination: unknown) { return destination; }
    read() { return null; }
  },
  Writable: class {
    on(event: string, listener: (...args: unknown[]) => void) { return this; }
    write(chunk: unknown) { return true; }
    end() {}
  },
  Transform: class {
    on(event: string, listener: (...args: unknown[]) => void) { return this; }
...
export const http = {
  ...
  request: (_options: unknown, callback?: (...args: unknown[]) => void) => {
    ...
    if (callback) { callback(new Error('http.request not supported')); }
    return {
      on: (_event: string, _cb: (...args: unknown[]) => void) => {return {};},
...
export const https = {
  ...
  request: (_options: unknown, callback?: (...args: unknown[]) => void) => {
    ...
    if (callback) { callback(new Error('https.request not supported')); }
    return {
      on: (_event: string, _cb: (...args: unknown[]) => void) => {return {};},
      write: (_chunk: unknown) => {},
      end: () => {}
