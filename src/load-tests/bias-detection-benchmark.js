...
function generateSessionData(baseSession) {
  ...
  return {
    session: {
      ...
      content: {
        patientPresentation: generatePatientPresentation(baseSession.scenarioType),
        ...
        sessionNotes: 'Productive session with good therapeutic rapport established.',
      },
...
function generatePatientPresentation(scenarioType) {
  const presentations = {
    depression: 'Patient presents with persistent low mood, fatigue, and social withdrawal for the past 6 weeks.',
    anxiety: 'Patient reports excessive worry, restlessness, and difficulty concentrating affecting daily activities.',
    trauma: 'Patient exhibits symptoms of PTSD following a recent traumatic event, including flashbacks and hypervigilance.',
    'substance-abuse': 'Patient acknowledges problematic alcohol use impacting work and relationships.',
  };
  
  return presentations[scenarioType] || 'Patient presents with general mental health concerns.';
}

function generateAIResponses() {
  const responses = [];
  const responseTypes = ['diagnostic', 'intervention', 'risk-assessment', 'recommendation'];
  
  for (let i = 0; i < randomIntBetween(2, 5); i++) {
    responses.push({
      responseId: `response-${randomString(8)}`,
      timestamp: new Date(Date.now() - randomIntBetween(0, 3600000)).toISOString(),
      type: responseTypes[randomIntBetween(0, responseTypes.length - 1)],
      content: 'AI-generated therapeutic response based on patient presentation.',
...
export default function () {
  ...
  const baseSession = SAMPLE_SESSIONS[randomIntBetween(0, SAMPLE_SESSIONS.length - 1)];
  ...
  group('Bias Detection Analysis', () => {
    ...
    const isSuccessful = check(response, {
      ...
      'response has analysis data': (r) => {
        try {
          const body = JSON.parse(r.body);
          return body.success && body.data && body.data.overallBiasScore !== undefined;
        } catch {
          return false;
        }
      },
  ...
  if (Math.random() < 0.1) { // 10% of requests test metrics endpoint
  ...
  if (Math.random() < 0.05) { // 5% of requests test health endpoint
    group('Health Check', () => {
...
export function teardown(data) {
  ...
  const finalMetrics = http.get(`${API_BASE}/metrics?timeRange=3600000&format=json`, {
  ...
  if (finalMetrics.status === 200) {
    try {
      ...
      console.log('Final performance summary:', JSON.stringify(metrics.summary, null, 2));
    } catch (e) {
