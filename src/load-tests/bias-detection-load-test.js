...
const sampleSessionData = {
  ...
  messages: [
    {
      role: 'user',
      content: 'I feel like everyone at work treats me differently because of my background.',
      timestamp: new Date().toISOString(),
    },
    {
      role: 'assistant',
      content: 'I understand that feeling different can be challenging. Can you tell me more about specific situations where you\'ve noticed this?',
      timestamp: new Date().toISOString(),
    },
    {
      role: 'user',
      content: 'During meetings, my ideas are often dismissed or ignored, but when others say similar things, they get positive responses.',
      timestamp: new Date().toISOString(),
    },
  ],
  metadata: {
...
function generateRandomSessionData() {
  ...
  return {
    ...
    messages: [
      ...
      {
        role: 'assistant',
        content: 'Thank you for sharing that with me. Can you tell me more about how this makes you feel?',
        timestamp: new Date().toISOString(),
      },
    ],
    metadata: {
  ...
  const success = check(response, {
    ...
    'analysis has bias score': (r) => {
      try {
        ...
        return data.biasScore !== undefined && typeof data.biasScore === 'number';
      } catch (e) {
        return false;
      }
    },
...
export function testRateLimiting() {
  ...
  for (let i = 0; i < 15; i++) {
    const response = http.get(`${BASE_URL}/api/bias-detection/health`, { headers });
...
function textSummary(data, options = {}) {
  ...
  const thresholdsMet = Object.entries(data.thresholds || {}).every(([name, result]) => !result.fails);
