...
describe('Session Analysis API Endpoint', () => {
  ...
  beforeEach(() => {
    ...
    // Setup utility mocks
    (validateTherapeuticSession as MockedFunction<typeof validateTherapeuticSession>)
  ...
  describe('POST /api/bias-detection/analyze', () => {
    ...
    const createMockResponse = (data: any, status: number = 200, headers: Record<string, string> = {}) => {
      ...
      return {
        ...
        headers: {
          get: vi.fn((key: string) => responseHeaders.get(key) || null)
        }
      };
    };
    ...
    it('should return cached result when available', async () => {
      mockCacheManager.analysisCache.getAnalysisResult.mockResolvedValue(mockAnalysisResult);
  ...
  describe('GET /api/bias-detection/analyze', () => {
    const createMockGetRequest = (searchParams: Record<string, string> = {}, headers: Record<string, string> = {}) => {
    ...
    it('should anonymize sensitive data when anonymize is true', async () => {
      ...
      expect(responseData.data.demographics.age).toBe(mockAnalysisResult.demographics.age);
      expect(responseData.data.demographics.gender).toBe(mockAnalysisResult.demographics.gender);
    });
    ...
    it('should handle bias detection engine errors in GET', async () => {
      ...
      // Verify bias engine was called and failed
      expect(mockBiasEngine.getSessionAnalysis).toHaveBeenCalledWith(mockSession.sessionId);
    });
