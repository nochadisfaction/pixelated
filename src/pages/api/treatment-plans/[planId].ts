...
const updateTreatmentGoalSchema = z.object({
  ...
  description: z.string().min(1).optional(),
  targetDate: z.string().datetime().optional().nullable(),
  status: z.enum(['Not Started', 'In Progress', 'Achieved', 'Partially Achieved', 'Not Achieved']).optional(),
...
export const GET: APIRoute = async ({ params, locals }) => {
  ...
  if (!supabase) return new Response(JSON.stringify({ error: 'Supabase client not found' }), { status: 500 });

  const { data: { user }, error: authError } = await supabase.auth.getUser();
  if (authError || !user) return new Response(JSON.stringify({ error: 'Not authenticated' }), { status: 401 });

  const planId = params.planId;
  if (!planId) return new Response(JSON.stringify({ error: 'Plan ID is required' }), { status: 400 });
  ...
  try {
    ...
    if (error) {
      if (error.code === 'PGRST116') { // PostgREST error for zero rows returned by .single()
        return new Response(JSON.stringify({ error: 'Treatment plan not found or not authorized.' }), { status: 404 });
      }
      throw error;
    }
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch treatment plan.', details: error instanceof Error ? error.message : 'Unknown error' }), { status: 500 });
  }
};
...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  ...
  try {
    ...
    if (!validationResult.success) {
      return new Response(JSON.stringify({ error: 'Invalid input data.', details: validationResult.error.flatten() }), { status: 400 });
    }
    ...
    if (!updatedPlanData) return new Response(JSON.stringify({ error: 'Plan not found or update failed.' }), { status: 404 });
    ...
    if (clientGoals) {
      for (const clientGoal of clientGoals) {
        ...
        if (clientObjectives && clientGoal.id) { // Only update objectives for existing goals in this simplified version
          for (const clientObjective of clientObjectives) {
            const objectiveData = { ...clientObjective }; 
            if (clientObjective.id) { // Existing objective
              ...
              if (objUpdateError) console.warn(`Error updating objective ${clientObjective.id}:`, objUpdateError);
            } else { // New objective
              const { error: objInsertError } = await supabase.from('treatment_objectives').insert({ ...objectiveData, goal_id: clientGoal.id, user_id: user.id });
              if (objInsertError) console.warn(`Error inserting new objective for goal ${clientGoal.id}:`, objInsertError);
            }
          }
        }
      }
    ...
    const { data: finalUpdatedPlan, error: fetchError } = await supabase
      .from('treatment_plans')
      .select(`id, client_id, therapist_id, title, diagnosis, start_date, end_date, status, general_notes, created_at, updated_at, goals:treatment_goals(id, description, target_date, status, created_at, updated_at, objectives:treatment_objectives(*))`)
      .eq('id', planId)
      .eq('user_id', user.id)
      .single();
    ...
    return new Response(JSON.stringify(finalUpdatedPlan as TreatmentPlan), { status: 200 });

  } catch (error) {
    console.error(`Error updating treatment plan ${planId}:`, error);
    return new Response(JSON.stringify({ error: 'Failed to update treatment plan.', details: error instanceof Error ? error.message : 'Unknown error' }), { status: 500 });
  }
};
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'Failed to delete treatment plan.', details: error instanceof Error ? error.message : 'Unknown error' }), { status: 500 });
  }
};
