...
import type { TreatmentPlan, TreatmentGoal, TreatmentObjective } from '../../../../types/treatment';
...
const treatmentGoalSchema = z.object({
  ...
  status: z.enum(['Not Started', 'In Progress', 'Achieved', 'Partially Achieved', 'Not Achieved']),
...
export const GET: APIRoute = async ({ locals }) => {
  ...
  try {
    ...
    return new Response(JSON.stringify(plans as TreatmentPlan[]), { status: 200 });
  } catch (error) {
    console.error('Error fetching treatment plans:', error);
    const message = error instanceof Error ? error.message : 'Unknown error';
    return new Response(JSON.stringify({ error: 'Failed to fetch treatment plans.', details: message }), { status: 500 });
  }
};
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const { data: { user }, error: authError } = await supabase.auth.getUser();
  if (authError || !user) return new Response(JSON.stringify({ error: 'User not authenticated.' }), { status: 401 });
  ...
  try {
    ...
    const { data: newPlanData, error: planError } = await supabase
      ...
      .select_one('id, client_id, therapist_id, title, diagnosis, start_date, end_date, status, general_notes, created_at, updated_at') // Select necessary fields
      .single();
    ...
    for (const clientGoal of clientGoals) {
      ...
      const { data: newGoalData, error: goalError } = await supabase
        ...
        .select_one('id, description, target_date, status, created_at, updated_at')
        .single();
      ...
      createdGoals.push({ ...newGoalData, objectives: createdObjectives } as TreatmentGoal);
    }
    ...
    const message = error instanceof Error ? error.message : 'Unknown error';
    return new Response(JSON.stringify({ error: 'Failed to create treatment plan.', details: message }), { status: 500 });
  }
}; 