...
program
  ...
  .action(async (options: { 
    ...
    try {
      const type = options.type as 'unauthorized_access' | 'data_leak' | 'system_compromise' | 'other'
      const severity = options.severity as 'low' | 'medium' | 'high' | 'critical'
  ...
  .action(async (options: {
    ...
    try {
      ...
      if (options.severity === 'critical' || userCount >= 500) {
        console.log(chalk.red('- Authorities (HHS) since breach affects 500+ users or is critical severity'))
      }

      console.log(chalk.blue('\nBreak details:'))
      console.log(chalk.blue('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'))
      console.log(`- Type: ${chalk.yellow(options.type)}`)
      console.log(`- Severity: ${getSeverityColor(options.severity as any)(options.severity)}`)
      ...
      if (options.severity === 'critical') {
        ...
        const doubleConfirm = await promptForConfirmation('Confirm you want to proceed with CRITICAL severity?')
      ...
      const breachId = await BreachNotificationSystem.reportBreach({
        type: options.type as 'unauthorized_access' | 'data_leak' | 'system_compromise' | 'other',
  ...
  .action(async (options: { limit: string }) => {
    try {
      ...
      breaches.slice(0, limit).forEach((breach, index) => {
        ...
        console.log(`   Status: ${getStatusColor(breach.notificationStatus)(breach.notificationStatus)}`)
        ...
        console.log(`   Description: ${chalk.gray(breach.description.substring(0, 100))}${breach.description.length > 100 ? '...' : ''}`)
        console.log(chalk.blue('───────────────────────────────────────────────────────────────────────────────'))
      })
