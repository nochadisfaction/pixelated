...
export interface AnalyticsEnvironmentVariables {
  ...
  readonly VERCEL_ANALYTICS_ID?: string | undefined
...
export interface BuildEnvironmentVariables {
  ...
  readonly VERCEL_URL?: Url | undefined
...
export function loadEnvironmentVariables(): ValidationResult<EnvironmentVariables> {
  ...
  const urlFields = ['SUPABASE_URL', 'SENTRY_DSN', 'VERCEL_URL'] as const
  ...
  const validatedEnv: EnvironmentVariables = {
    ...
    // Build variables
    VERCEL: env['VERCEL'],
    VERCEL_URL: env['VERCEL_URL'] as Url | undefined,
    VERCEL_ENV: env['VERCEL_ENV'] as
      | 'production'
      | 'preview'
      | 'development'
      | undefined,
...
export const isVercel = (): boolean => Boolean(process.env['VERCEL'])
