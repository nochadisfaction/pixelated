...
describe('Bias Detection API Integration Tests', () => {
  ...
  const validTestSession: TestSession = {
    ...
    conversationData: {
      messages: [
        ...
        {
          role: 'assistant',
          content: 'Your accent is part of who you are, and it doesn\'t diminish your expertise or value. Let\'s work on building your confidence.',
  ...
  describe('POST /api/bias-detection/analyze', () => {
    ...
    it('should successfully analyze a valid session', async () => {
      ...
      expect(data.data.demographics).toEqual(testSession.participantDemographics);
  ...
  describe('GET /api/bias-detection/analyze', () => {
    ...
    it('should anonymize sensitive data when anonymize is true', async () => {
      ...
      expect(data.data.demographics.age).toBe(testSession.participantDemographics.age);
      expect(data.data.demographics.gender).toBe(testSession.participantDemographics.gender);
    });
  ...
  describe('Cross-Endpoint Integration Scenarios', () => {
    ...
    it('should handle authentication consistently across all endpoints', async () => {
      const endpoints = [
        { method: 'POST', url: `${testServer.baseUrl}/api/bias-detection/analyze`, body: { session: testSession } },
        { method: 'GET', url: `${testServer.baseUrl}/api/bias-detection/analyze?sessionId=${testSession.sessionId}` },
        { method: 'GET', url: `${testServer.baseUrl}/api/bias-detection/dashboard` },
        { method: 'GET', url: `${testServer.baseUrl}/api/bias-detection/export` }
      ];
